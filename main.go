...
type InfoMessage struct {
	...
	Distance     float64       // расстояние в километрах, которое преодолел пользователь
	Speed        float64       // средняя скорость в км/ч, с которой двигался пользователь
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	averageSpeed := CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift
	result := averageSpeed * r.Weight / MInKm * r.Duration.Hours() * MinInHours
	return result
}
...
func (w Walking) Calories() float64 {
	...
	powSpeed := math.Pow(speedInMsec, 2)
	durationInMin := w.Duration.Hours() * MinInHours
	heightInM := w.Height / CmInM
	result := (CaloriesWeightMultiplier*w.Weight + (powSpeed/heightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * durationInMin
	return result
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	swimingSpeed := s.meanSpeed() + SwimmingCaloriesMeanSpeedShift
	result := swimingSpeed * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return result
}
